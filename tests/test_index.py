...
class LandingContentTests(unittest.TestCase):
    def test_uses_internal_reservation_route_and_confirmed_identity(self):
        self.assertEqual(HTML.count('href="/reservar"'), 3)
    ...
    def test_presents_the_reservation_as_an_interactive_demo(self):
        self.assertIn(
            'content="Evolution Studio en Quinta de Tilcoco. Explora una demo interactiva de reserva dentro del sitio."',
            HTML,
        )
        self.assertIn("Explora la demo interactiva de reserva", HTML)
        self.assertIn("Continúa dentro del sitio", HTML)
